    ...
    return(
        <div className="p-4 flex flex-col gap-6 min-h-screen">
            ...
            <div className="md:flex-1">
              ...
              <div className="text-gray-700 space-y-1">
                ...
                <p>
                  ...
                  <a href={selectedOrder?.trackingUrl} className="text-sky-950 underline ml-1">{selectedOrder?.trackingNumber}</a>
                </p>
              </div>
            </div>
          </div>
          ...
          <div className="flex flex-col border rounded-lg p-4">
            ...
            {/* Cabeçalho */}
            <div className="hidden md:flex w-full font-semibold text-gray-600 mb-2">
              <div className="w-3/5">Item</div>
              <div className="w-1/5">Preço</div>
              <div className="w-1/5">Quantidade</div>
            ...
            <div className="flex flex-col gap-6">
              {selectedOrder?.products.map((product) => (
                <div key={product.id} className="flex flex-col md:flex-row w-full items-start md:items-center gap-4">
                  ...
                  <div className="flex w-full md:w-2/5 justify-between md:justify-normal md:gap-10 text-gray-800">
                    <span className="md:w-1/3">R$ {product.price.toFixed(2)}</span>
                    <span className="md:w-1/3">{product.amount}</span>
                    <span className="md:w-1/3">R$ {(product.amount * product.price).toFixed(2)}</span>
                  </div>
                </div>
              ))}
            </div>
            ...
            {/* Total */}
            <div className="mt-6 flex justify-end font-semibold text-lg text-gray-800">
              <span>Total da compra: R$ {selectedOrder?.price.toFixed(2)}</span>
            </div>
          </div>
        </div>
    )}